...
class unidirected_graph{
    ...
    map<T, set<T>> lista_adiacenza; //(lista di adiacenza)  map<T,map<T,double>> AGGIUNGO I PESI AL GRAFO
...
public:
    ...
    /*vector<unidirected_edge<T>> operator-(const unidirected_graph<T>& other) const
    {
...
vector<unidirected_edge<T>> operator-(const unidirected_graph& other) const {
    ...
    for (size_t i=0;i<archi.size();i++){
        auto iteratore=(find(archi_other.begin(),archi_other.end(),archi[i]));

        if (iteratore==archi_other.end()){
            differenza.push_back(archi[i]);
        }
    }
    return differenza;
