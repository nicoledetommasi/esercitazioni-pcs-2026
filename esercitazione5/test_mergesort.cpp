...
int main() {
    ...
    for (int i=0;i<100;i++){
        ...
        merge_sort(casuali,0,casuali.size()-1);
        if (!is_sorted(casuali.begin(),casuali.end())){
            std::cerr<< "Il mergesort non ha ordinato bene il vettore \n";
            return EXIT_FAILURE;
    ...
    if (!is_sorted(stringa)){
            std::cerr<< "Il mergesort non ha ordinato bene il vettore di stringhe \n";
            return EXIT_FAILURE;
